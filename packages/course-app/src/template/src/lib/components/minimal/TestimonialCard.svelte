<script lang="ts">
  import defaultImage from './assets/classroomio-course-img-template.jpg';
  interface Props {
    name?: string;
    description?: string;
    role?: string;
  }

  let { name = '', description = '', role = '' }: Props = $props();
</script>

<section
  class="border border-[#D7D7D7] px-4 py-6 space-y-4 bg-[#FDFDFD] rounded w-full max-w-[350px] md:max-w-[300px]"
>
  <div class="flex items-center gap-2">
    <img src={defaultImage} alt="student" class="w-12 h-12 rounded-full" />
    <span>
      <p class="text-sm font-bold">{name}</p>
      <p class="text-xs text-[#696969]">{role}</p>
    </span>
  </div>
  <p class="text-[#696969] text-justify line-clamp-5">
    {description}
  </p>
</section>
