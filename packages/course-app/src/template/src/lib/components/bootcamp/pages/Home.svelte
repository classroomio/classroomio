...
<main class="bg-black/90">
  ...
  {#if $homePage}
    ...
    {#if content?.show}
      <section class="flex min-h-full items-center justify-center px-10 py-10 md:px-14">
        <section class="flex flex-col items-center gap-10 text-center">
          ...
          <PrimaryButton
            ...
            onClick={() => goto(content.settings.action.link)}
          />
        </section>
      </section>
    {/if}
  ...
  {#if aboutSection?.show}
    <section id="aboutSectionus" class="relative w-full bg-white px-4 py-10 lg:px-14">
      <div class="mx-4 flex flex-col items-center justify-between gap-4 md:flex-row md:items-start">
        <div class="mb-5 w-full md:mr-5 md:w-[60%] lg:mb-0 lg:w-[50%]">
          ...
          <div class="space-y-3">
            {#each aboutSection.settings.benefits.list as items}
              <div
                class="h-[12rem] overflow-hidden rounded-sm border border-gray-100 bg-gray-50 p-3"
              >
                <p class="font-slab mb-2 line-clamp-2 w-[90%] text-xl md:text-2xl">{items.title}</p>
                <p
                  class="font-roboto mb-4 line-clamp-5 w-[90%] text-sm text-gray-400 lg:line-clamp-4 lg:text-base"
                >
                  {items.subtitle}
                </p>
              </div>
    ...
    <section id="course" class="h-full bg-white p-4 lg:p-20">
      <div class="mx-4 mb-4">
        <span class="mb-4 space-y-2 lg:mb-0">
          ...
          <p class=" font-roboto w-[80%] text-gray-400">{coursesSection.settings.subtitle}</p>
        </span>
      </div>
    ...
    <section class="bg-bootcamp flex flex-col items-center justify-between px-6 py-20 lg:px-10">
      ...
      <div class="my-5 flex w-full items-center justify-center">
        <PrimaryButton
          ...
          class="rounded-none bg-white text-lg font-bold uppercase text-black"
        />
      </div>
    </section>
  {/if}
