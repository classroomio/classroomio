--> Give existing MCP keys the course member scopes that new MCP keys get by default, and drop public_api:*
--> from them, so an MCP key only reaches the course member and invite routes of the public API.
...
SET "scopes" = ("scopes" - 'public_api:*') || '["course:member:read", "course:member:write"]'::jsonb
WHERE "type" = 'mcp'
  ...
  AND NOT "scopes" @> '["course:member:read"]'::jsonb;
