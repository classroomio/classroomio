...
export async function createExercises(values: TNewExercise[]) {
  try {
    return await db.insert(schema.exercise).values(values).returning();
  } catch (error) {
...
export async function getExercisesByLessonIds(lessonIds: string[]): Promise<TExercise[]> {
  ...
  try {
    return await db.select().from(schema.exercise).where(inArray(schema.exercise.lessonId, lessonIds));
  } catch (error) {
    throw new Error(`Failed to get exercises by lesson IDs: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }
}
...
export async function createQuestions(values: TNewQuestion[]) {
  ...
  try {
    return await db.insert(schema.question).values(values).returning();
  } catch (error) {
...
export async function getQuestionsByExerciseIds(exerciseIds: string[]): Promise<TNewQuestion[]> {
  ...
  try {
    return await db.select().from(schema.question).where(inArray(schema.question.exerciseId, exerciseIds));
  } catch (error) {
    throw new Error(`Failed to get questions by exercise IDs: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }
}
...
export async function createOptions(values: TNewOption[]) {
  if (values.length === 0) return Promise.resolve([]);
  try {
    return await db.insert(schema.option).values(values).returning();
  } catch (error) {
...
export async function getOptionsByQuestionIds(questionIds: number[]) {
  ...
  try {
    return await db.select().from(schema.option).where(inArray(schema.option.questionId, questionIds));
  } catch (error) {
    throw new Error(`Failed to get options by question IDs: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }
}
