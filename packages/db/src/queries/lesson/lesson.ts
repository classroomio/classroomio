...
export async function getSectionsByCourseId(courseId: string) {
  try {
    return await db.select().from(schema.lessonSection).where(eq(schema.lessonSection.courseId, courseId));
  } catch (error) {
    throw new Error(`Failed to get sections by course ID "${courseId}": ${error instanceof Error ? error.message : 'Unknown error'}`);
  }
}

export async function createLessonSections(values: TNewLessonSection[]) {
  try {
    return await db.insert(schema.lessonSection).values(values).returning();
  } catch (error) {
...
export async function getLessonsByCourseId(courseId: string) {
  try {
    return await db.select().from(schema.lesson).where(eq(schema.lesson.courseId, courseId));
  } catch (error) {
    throw new Error(`Failed to get lessons by course ID "${courseId}": ${error instanceof Error ? error.message : 'Unknown error'}`);
  }
}

export async function createLessons(values: TNewLesson[]) {
  try {
    return await db.insert(schema.lesson).values(values).returning();
  } catch (error) {
...
export async function getLessonLanguagesByLessonIds(lessonIds: string[]) {
  ...
  try {
    return await db.select().from(schema.lessonLanguage).where(inArray(schema.lessonLanguage.lessonId, lessonIds));
  } catch (error) {
    throw new Error(`Failed to get lesson languages by lesson IDs: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }
}
...
export async function createLessonLanguages(values: TNewLessonLanguage[]) {
  ...
  try {
    return await db.insert(schema.lessonLanguage).values(values).returning();
  } catch (error) {
