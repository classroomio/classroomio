import { and, db, eq, inArray, isNull, notInArray, orgDoc, orgDocFavorite, orgDocShare } from '@db/drizzle';
...
function buildFavorites(adminUserId: string): TNewOrgNoteFavorite[] {
  return FAVORITE_NOTE_IDS.map((docId) => ({
...
function buildShares(adminUserId: string, studentUserId: string): TNewOrgNoteShare[] {
  ...
  return sharedNoteIds.map((docId) => ({
    docId,
    profileId: adminUserId,
...
export async function seedDocs({
  ...
  const cleaned = await db
    ...
    .where(
      and(
        eq(orgDoc.organizationId, testOrgId),
        eq(orgDoc.origin, 'organization'),
        eq(orgDoc.isTemplate, false),
        isNull(orgDoc.deletedAt),
        notInArray(orgDoc.id, NON_TEMPLATE_SEED_NOTE_IDS)
      )
    )
    .returning({ id: orgDoc.id });
  ...
  const existing = await db
    .select({ id: orgDoc.id })
    .from(orgDoc)
    .where(inArray(orgDoc.id, SEED_NOTE_IDS));

  if (existing.length > 0) {
    console.log('   ✓ Notes demo data already exists, syncing favorites');
    await db.delete(orgDocFavorite).where(eq(orgDocFavorite.profileId, adminUserId));
    const favorites = buildFavorites(adminUserId);
    await db.insert(orgDocFavorite).values(favorites);
  ...
  await db.insert(orgDoc).values(notes);
  ...
  await db.insert(orgDocShare).values(shares);
