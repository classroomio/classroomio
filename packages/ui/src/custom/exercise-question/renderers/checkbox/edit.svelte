...
<div class="ui:space-y-3">
  <p class="ui:text-muted-foreground ui:text-xs">{label('checkbox.edit.helper')}</p>
  {#if optionImageError}
      ...
      <div class="ui:flex ui:items-start ui:gap-2">
        <IconButton
          ...
          tooltip={option.isCorrect === true
            ? label('checkbox.edit.correct_selected_tooltip')
            : label('checkbox.edit.toggle_correct_tooltip')}
        ...
        >
          ...
          {/if}
          <span class="ui:sr-only">{label('checkbox.edit.toggle_correct_sr')}</span>
        </IconButton>
