...
<EditableLandingSection sectionKey="navigation">
  <header
    class="ui:flex ui:items-center ui:justify-between ui:gap-4 ui:py-2 ui:pl-5 ui:pr-2 ui:rounded-full ui:border ui:border-border ui:bg-background/85 ui:backdrop-blur-md ui:shadow-[0_4px_24px_-8px_rgba(14,14,16,0.08)]"
  >
    <a
      href="/"
    ...
    {#if navItems.length > 0}
      <nav class="ui:hidden ui:md:flex ui:gap-1">
        {#each navItems as item (item.href + item.label)}
          <a
            href={item.href}
            class="ui:px-3.5 ui:py-1.5 ui:text-sm ui:font-medium ui:text-foreground/80 ui:hover:text-foreground ui:hover:bg-muted/60 ui:rounded-full ui:transition-colors ui:no-underline"
          >
            {item.label}
          </a>
        {/each}
