<script lang="ts">
  ...
  const sideBySide = $derived(Boolean(hero.image) && !centered);
</script>
...
<EditableLandingSection sectionKey="hero">
  <header class="ui:bg-[var(--landing-bg)]">
    <div class="ui:max-w-[1200px] ui:mx-auto ui:px-5 ui:md:px-8 {compact ? 'ui:pt-10 ui:pb-8' : 'ui:pt-16 ui:pb-12'}">
      {#if sideBySide}
        <!-- Copy and image share the row; the image falls under the copy below lg. -->
        <div
          class="ui:grid ui:grid-cols-1 ui:lg:grid-cols-[minmax(0,1fr)_minmax(0,440px)] ui:gap-10 ui:lg:gap-14 ui:lg:items-center"
        >
          <div>{@render copy()}</div>
          {@render image()}
        </div>
      {:else}
        {@render copy()}
        {#if hero.image}
          <div class="ui:mt-12 ui:flex ui:justify-center">{@render image()}</div>
        {/if}
      {/if}
    </div>
  </header>
  ...
  {#if showActions}
    <div class="ui:mt-8 ui:flex ui:flex-wrap ui:items-center ui:gap-2.5 {centered ? 'ui:justify-center' : ''}">
      ...
      {#if hero.secondaryAction}
        <SecondaryActionButton href={hero.secondaryAction.href} label={hero.secondaryAction.label} variant="quartz" />
      {/if}
    </div>
  {/if}
