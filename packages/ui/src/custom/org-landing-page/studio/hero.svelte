  ...
  <section class="ui:relative ui:overflow-hidden ui:px-6 ui:pt-20 ui:pb-20 ui:md:pt-24 ui:md:pb-24">
    ...
    <div class="ui:relative ui:max-w-[1080px] ui:mx-auto">
      <BlurFade delay={0} once={true}>
        <span
          class="ui:inline-flex ui:items-center ui:gap-2 ui:mb-7 ui:px-3 ui:py-1.5 ui:rounded-full ui:border ui:border-border ui:bg-card/70 ui:backdrop-blur-sm ui:text-[12px] ui:font-medium ui:text-foreground"
        >
      ...
      <BlurFade delay={0.08} once={true}>
        <h1
          class="ui:text-4xl ui:md:text-5xl ui:lg:text-[64px] ui:font-semibold ui:tracking-tight ui:leading-[1.02] ui:max-w-[18ch] ui:mb-5 ui:text-foreground ui:text-balance"
        >
          {hero.heading}
        </h1>
      </BlurFade>
      ...
      {#if hero.image}
        <BlurFade delay={0.4} once={true}>
          <div
            class="studio-polaroid ui:mt-16 ui:relative ui:rounded-xl ui:border ui:border-border ui:bg-card ui:p-2 ui:shadow-[0_40px_90px_-30px_rgba(0,0,0,0.35)]"
          >
            <img src={hero.image} alt="" class="ui:w-full ui:h-auto ui:rounded-lg ui:border ui:border-border" />
          </div>
        </BlurFade>
      {/if}

      {#if showMarquee}
        <BlurFade delay={hero.image ? 0.5 : 0.4} once={true}>
          <div class="studio-marquee-mask ui:mt-16 ui:py-4 ui:border-y ui:border-border" aria-hidden="true">
            <div class="studio-marquee-track ui:flex ui:gap-12 ui:whitespace-nowrap ui:w-max">
              {#each [...marqueeItems, ...marqueeItems] as course, index (`${course.id}-${index}`)}
                <span class="ui:inline-flex ui:items-center ui:gap-3">
                  <span class="ui:size-1 ui:rounded-full ui:bg-primary/70"></span>
                  <span class="ui:text-sm ui:font-medium ui:text-muted-foreground">
                    {course.title}
                  </span>
                </span>
              {/each}
