<script lang="ts">
  ...
  let {
    ...
    sideOffset = 4,
    portalProps,
    // bits-ui defaults this to true, which locks body scroll on every open:
    // a forced full-document style recalc + reflow, plus pointer-events: none.
    // A menu doesn't need it; only modal layers do.
    preventScroll = false,
    class: className,
    ...restProps
