...
export function getNoteCommentsChannel(noteId: string) {
  return `note:${noteId}:comments`;
}
...
export async function publishNoteCommentEvent(
  noteId: string,
  event: Omit<NoteCommentStreamEvent, 'noteId'>
): Promise<void> {
  ...
  try {
    await redis.publish(getNoteCommentsChannel(noteId), JSON.stringify({ ...event, noteId }));
  } catch (error) {
...
export async function subscribeNoteCommentEvents(
  ...
  const channel = getNoteCommentsChannel(noteId);
