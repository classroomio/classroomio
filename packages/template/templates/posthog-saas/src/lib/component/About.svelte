...
{#if data.aboutUs.show}
  <section id="about" class="px-4 md:px-10 pt-4 pb-20 h-full w-full mx-auto md:w-[90%] lg:w-[80%]">
    <h1 class="text-start text-3xl mb-2 font-bold md:w-[50%]">{data.aboutUs.title}</h1>
    <p class="text-start text-[#3D3D3D] dark:text-white text-sm font-semibold md:w-[70%] mb-8">
      {data.aboutUs.subtitle}
    </p>
    <div>
      <p class="text-xl font-bold capitalize mb-6">{data.aboutUs.benefits.title}</p>
