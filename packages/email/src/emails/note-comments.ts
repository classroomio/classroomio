...
export const noteCommentMentionEmail = defineEmail({
  ...
  render: (fields) => {
    const content = `
      <p>${fields.authorName} mentioned you in a comment on <strong>${fields.noteTitle}</strong>.</p>
...
export const noteCommentReplyEmail = defineEmail({
  ...
  render: (fields) => {
    const content = `
      <p>${fields.authorName} replied in a comment thread on <strong>${fields.noteTitle}</strong>.</p>
      <div style="font-style: italic; margin-top: 10px;">${fields.comment}</div>
      <div>
