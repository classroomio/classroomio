export interface TranscriptVttSegment {
  start: number;
  end: number;
  text: string;
}
...
/**
 * Build a minimal WEBVTT caption file from transcript segment timestamps.
 */
export function segmentsToWebVtt(segments: TranscriptVttSegment[]): string {
  const lines = ['WEBVTT', ''];
