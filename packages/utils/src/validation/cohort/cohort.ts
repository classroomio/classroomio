...
export type TAddCourseToCohort = z.infer<typeof ZAddCourseToCohort>;

export const ZAddCohortMembers = z.object({
  members: z
    .array(
      z.object({
...
export const ZUpdateCohortMember = z.object({
  roleId: z.union([z.literal(ROLE.TUTOR), z.literal(ROLE.STUDENT)])
});
export type TUpdateCohortMember = z.infer<typeof ZUpdateCohortMember>;

export const ZCreateCohortNewsfeed = z.object({
...
export type TUpdateCohortReaction = z.infer<typeof ZUpdateCohortReaction>;

export const ZCreateCohortNewsfeedComment = z.object({
  content: z.string().min(1)
});
export type TCreateCohortNewsfeedComment = z.infer<typeof ZCreateCohortNewsfeedComment>;

export const ZInviteStudentsToCohort = z.object({
  recipientCsv: z.string().min(1).max(25000),
  sendEmail: z.boolean().default(true)
});
export type TInviteStudentsToCohort = z.infer<typeof ZInviteStudentsToCohort>;

export const ZAssignExistingStudentsToCohort = z.object({
...
export const COHORT_GOAL_DEADLINE_KINDS = ['absolute', 'relative_to_join', 'recurring', 'none'] as const;
...
const goalDeadlineFields = {
  deadlineKind: z.enum(COHORT_GOAL_DEADLINE_KINDS).default('none'),
 ...
 */
export const ZCreateCohortGoal = z
  .object({
    type: z.enum(COHORT_GOAL_TYPES),
...
export type TCreateCohortGoal = z.infer<typeof ZCreateCohortGoal>;

export const ZUpdateCohortGoal = z
  .object({
    type: z.enum(COHORT_GOAL_TYPES).optional(),
    ...
    deadlineKind: z.enum(COHORT_GOAL_DEADLINE_KINDS).optional(),
    deadlineDate: z.string().datetime().nullable().optional(),
    relativeDays: z.number().int().min(1).max(3650).nullable().optional(),
    recurringMonths: z.number().int().min(1).max(120).nullable().optional(),
