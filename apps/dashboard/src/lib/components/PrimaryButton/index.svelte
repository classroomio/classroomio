<script lang="ts">
  ...
  $: loadingClass = isLoading || isDisabled ? 'cursor-not-allowed opacity-25' : `cursor-pointer ${
        !disableScale && 'hover:scale-95'
      }`;
  $: cname = `flex items-center h-auto ${loadingClass} ${VARIANTS_CLASS[isLoading ? VARIANTS.OUTLINED : variant]} ${!disablePadding &&
    'px-5 py-[0.2rem]'} rounded-md min-h-[36px] w-fit justify-center sm:w-auto ${variant !==
    VARIANTS.TEXT && 'hover:shadow-xl'} transition-all delay-150 duration-300 ease-in-out`;
</script>

<button
  class={cn(cname, className)}
  on:click={onClick}
  {name}
  {type}
