<script>
  ...
  const handleAddNewComment = (event) => {
    if (event.key === 'Enter' || event.type === 'click') {
      ...
      addNewComment(comment, value.id);
      comment = '';
    }
  };
        ...
        <div class="relative">
          ...
          {#if isAnouncementOptionOpen == true}
            <div class="absolute right-2 rounded-md p-2 border hover:bg-slate-200 cursor-pointer">
              <button on:click={() => handleDeleteAnouncement('announcement')}>
                <p>Delete</p>
              </button>
            </div>
          {/if}
    ...
    {#if value.comments.length > 0}
      ...
      >
        ...
        <p class="text-sm py-2">
          {value.comments.length} class {value.comments.length > 1 ? 'comments' : 'comment'}
        </p>
      </button>
    {/if}
    <div>
      {#each value.comments as comment, index}
        {#if comment.content && (areCommentsExpanded || index === value.comments.length - 1)}
