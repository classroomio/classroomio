<script>
  import Calendar from 'carbon-icons-svelte/lib/Calendar.svelte';
</script>

<div
  class=" border border-[#EAEAEA] bg-[#FCFCFC] dark:border-neutral-600 rounded w-full max-w-[300px] space-y-4 p-4"
>
  <img
    src="/images/classroomio-course-img-template.jpg"
    alt="tutor"
    class="object-cover w-full h-full"
  />
  <div class="w-full space-y-2">
    <p class="text-lg font-semibold line-clamp-2">
      How to Choose the Right Online Course to Boost Your Career
    </p>
    <span class="inline-flex items-center gap-1 text-[#282828]">
      <Calendar class="fill-black" />
      <p class="text-xs">Mon Jun 05, 2023</p>
    </span>
  </div>
</div>
