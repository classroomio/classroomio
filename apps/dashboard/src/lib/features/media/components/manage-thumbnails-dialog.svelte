...
>
  <Dialog.Content class="max-h-[85vh] w-[95%] max-w-3xl overflow-y-auto">
    <Dialog.Header>
      ...
      <div class="space-y-5 p-1">
        <div class="ui:bg-muted/40 aspect-video w-full overflow-hidden rounded-md border">
          ...
          {:else if videoUrl}
            ...
            <video controls preload="metadata" src={videoUrl} class="h-full w-full bg-black"></video>
          {:else}
    ...
    <Dialog.Footer class="ui:gap-2 flex-col sm:flex-row sm:justify-between">
      <Button variant="outline" onclick={regenerate} loading={isRegenerating} disabled={isSaving}>
