...
class PublicConversionStore {
  ...
  syncExerciseQuestions(
    ...
    const otherInitialOffenders = this.initialOffenders.filter((o) => o.exerciseId !== exerciseId);
    ...
    const prevInitialForExercise = this.initialOffenders.filter((o) => o.exerciseId === exerciseId);
    if (newOffenders.length > prevInitialForExercise.length) {
      this.initialOffenders = [...otherInitialOffenders, ...newOffenders];
    }
  ...
  getGroups(course?: Course | null): ExerciseConversionGroup[] {
    const allOffenders = getAllKnownOffenders(this.initialOffenders, this.offenders);
    return groupOffendersByExercise(allOffenders, course);
  }
