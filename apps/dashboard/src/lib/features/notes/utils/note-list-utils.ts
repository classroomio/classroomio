...
export function notePreviewText(note: NoteListItem) {
  const preview = note.plainText?.trim();

  if (!preview) {
    return t.get('notes.list.no_content');
  }

  return preview;
}

export function formatRecentNoteMeta(updatedAt: string, note: NoteListItem) {
...
export function displayNoteTitle(title: string) {
  ...
  const defaultTitle = t.get('notes.org.new_note_title');
...
function sectionLabelForDate(date: Date, now: Date) {
  ...
  if (diffDays <= 7) {
    return t.get('notes.workspace.sections.previous_7_days');
  }

  if (diffDays <= 30) {
    return t.get('notes.workspace.sections.previous_30_days');
  }
...
export function buildNoteListSections(notes: NoteListItem[]): NoteListSection[] {
  ...
  if (pinnedNotes.length > 0) {
    sections.push({
      id: 'pinned',
      label: t.get('notes.workspace.sections.pinned'),
      notes: pinnedNotes
    });
  }
