...
<Dialog.Root bind:open {onOpenChange}>
  <Dialog.Content class="max-w-2xl">
    <Dialog.Header>
      <Dialog.Title>{$t('notes.page.add_cover')}</Dialog.Title>
      <Dialog.Description>{$t('notes.page.add_cover_description')}</Dialog.Description>
    </Dialog.Header>
    ...
    <div class={isUploading ? 'pointer-events-none opacity-60' : ''}>
      ...
      >
        ...
        >
          ...
          {/if}
          <p class="ui:m-0 ui:text-sm ui:font-medium">{$t('notes.page.upload_cover_prompt')}</p>
        </ImageCropper.UploadTrigger>
