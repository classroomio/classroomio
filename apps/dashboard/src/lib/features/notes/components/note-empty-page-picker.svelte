<script lang="ts">
  ...
  const items: PickerItem[] = [
    { id: 'empty', label: 'notes.empty_page.empty', icon: FileIcon },
    { id: 'table_of_contents', label: 'notes.empty_page.with_table_of_contents', icon: ListTreeIcon },
    { id: 'templates', label: 'notes.empty_page.templates', icon: LayoutTemplateIcon },
    { id: 'import', label: 'notes.empty_page.import', icon: UploadIcon }
  ];
...
<div class={cn('flex flex-col gap-3 py-1', className)}>
  <p class="ui:text-muted-foreground text-sm leading-relaxed">
    {$t('notes.empty_page.hint')}
  </p>

  <ul class="flex flex-col gap-0.5" role="listbox" aria-label={$t('notes.empty_page.aria_label')}>
