<script lang="ts">
  ...
  async function loadLessonNote() {
    ...
    const note = await notesApi.ensureLessonCaptureNote({
      courseId,
      lessonId,
      title: lessonTitle
    });
    ...
    if (!note) {
      loadError = t.get('notes.lesson_panel.load_error');
      isLoading = false;
...
<div class="flex min-h-0 flex-1 flex-col">
  <header class="border-border flex items-start justify-between gap-2 border-b px-4 py-3">
    <div class="min-w-0">
      ...
      <a href={lessonHref} class="ui:text-muted-foreground ui:hover:text-foreground truncate text-xs underline">
        {$t('notes.lesson_panel.view_lesson')}
      </a>
    </div>
  ...
  <div class="flex items-center gap-2 border-b px-4 py-2">
    <Button variant="secondary" size="sm" disabled={!canInsertTimestamp} onclick={insertCurrentTimestamp}>
      <ClockIcon size={14} />
      {$t('notes.lesson_panel.insert_timestamp')}
    </Button>
    {#if !canInsertTimestamp}
      <span class="ui:text-muted-foreground text-xs">{$t('notes.lesson_panel.timestamp_hint')}</span>
    {/if}
  </div>
