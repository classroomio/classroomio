<script lang="ts">
  ...
  import { PathCard, PathRow, PathFilterPopover, CreatePathDialog, ClonePathModal } from '../components';
  ...
  let { basePath, initialPaths }: Props = $props();

  $effect.pre(() => {
    if (initialPaths && initialPaths.length > 0 && learningPathApi.paths.length === 0) {
      learningPathApi.paths = [...initialPaths];
  ...
  const filteredPaths = $derived.by(() => {
    ...
    if (statusFilter === 'published') {
      list = list.filter((p) => p.status === 'ACTIVE' || (p.status as string) === 'PUBLISHED');
    ...
    sorted.sort((a, b) => {
      ...
      if (sortKey === 'date_created') {
        ...
        const aPub = a.status === 'ACTIVE' ? 1 : 0;
        const bPub = b.status === 'ACTIVE' ? 1 : 0;
...
<CreatePathDialog bind:open={showCreateDialog} onClose={handleCloseCreateDialog} onCreated={handleCreated} />
  ...
  {:else if viewMode === 'grid'}
    <Item.Group class="grid! w-full grid-cols-1 justify-items-center gap-4 md:grid-cols-2 xl:grid-cols-3">
      {#each filteredPaths as path (path.id)}
        <PathCard {path} {basePath} onDelete={handleRequestDelete} />
      {/each}
    </Item.Group>
  {:else}
    <ResourceListRow.Group class="@container">
      {#each filteredPaths as path (path.id)}
        <PathRow {path} {basePath} onDelete={handleRequestDelete} />
      {/each}
