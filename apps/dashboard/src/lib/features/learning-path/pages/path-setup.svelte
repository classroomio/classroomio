...
<Page.Root class="mx-auto flex w-[90%] px-4 pb-12 md:max-w-2xl lg:max-w-3xl">
  <Page.Header>
    <Page.HeaderContent>
      ...
      <Page.Subtitle>
        {$t('learningPath.setup.subtitle', { name: path.name })}
      </Page.Subtitle>
    </Page.HeaderContent>
    <Page.Action>
      ...
      <div class="ui:border-border ui:bg-card mb-4 rounded-lg border p-4">
        ...
        <div class="mt-2.5 flex items-center gap-3">
          <span class="ui:text-foreground text-xs tabular-nums">
            {$t('learningPath.setup.progress', { completed: progress.completed, total: progress.total })}
          </span>
                ...
                <div class="flex items-center gap-1.5">
                  ...
                  {#if step.id === 'order'}
                    <Button
                      variant="ghost"
                      size="sm"
                      class="ui:text-muted-foreground hover:ui:text-foreground hover:ui:bg-accent h-8 px-2 text-xs font-medium"
                      onclick={() => goto(step.href)}
                    >
                      {$t('learningPath.setup.reorder_action')}
          ...
          <div class="flex items-center gap-3.5">
            ...
            <div>
              <p class="ui:text-foreground text-sm font-semibold">
                {$t('learningPath.setup.launch_title', { count: stepsLeft })}
              </p>
