 ...
 */
export function getStatusBadgeVariant(status: LearningPathStatus | string): 'success' | 'secondary' {
  return status === 'ACTIVE' || (status as string) === 'PUBLISHED' ? 'success' : 'secondary';
}

/**
 * Human-readable label for status
 */
export function getStatusDisplayLabel(status: LearningPathStatus | string): string {
  return status === 'ACTIVE' || (status as string) === 'PUBLISHED' ? 'Published' : 'Unpublished';
}
