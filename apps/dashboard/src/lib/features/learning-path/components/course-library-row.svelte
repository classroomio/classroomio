...
<div
  class="hover:ui:border-ring flex items-center gap-4 rounded-xl border p-4 shadow-sm transition-[box-shadow,border-color] hover:shadow-md"
>
  <a
    {href}
    class="flex h-24 w-28 shrink-0 items-center justify-center overflow-hidden rounded-lg"
    style="background: {coverGradient}"
    aria-label={title}
  >
    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" class="ui:text-white/30 size-8" aria-hidden="true">
  ...
  <div class="min-w-0 flex-1">
    ...
    <a {href} class="w-fit">
      <h3 class="hover:ui:text-primary truncate text-sm font-semibold">{title}</h3>
    </a>
  ...
  <div class="flex w-40 shrink-0 flex-col items-end gap-2">
    <div class="flex w-full items-center gap-2">
      ...
      <span class="tnum w-9 shrink-0 text-right text-xs font-semibold {done ? 'ui:text-emerald-600' : ''}"
