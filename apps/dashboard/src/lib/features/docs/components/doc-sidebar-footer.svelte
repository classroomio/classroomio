...
<footer class={cn('border-border mt-auto space-y-0.5 border-t px-1 py-2', className)}>
  <a href={templatesHref} class={footerLinkClass(pathname.endsWith('/docs/templates'))}>
    <LayoutTemplateIcon size={16} />
    {$t('docs.sidebar.footer.templates')}
  </a>
  <a href={trashHref} class={footerLinkClass(pathname.endsWith('/docs/trash'))}>
    <Trash2Icon size={16} />
    {$t('docs.sidebar.footer.trash')}
  </a>
</footer>
