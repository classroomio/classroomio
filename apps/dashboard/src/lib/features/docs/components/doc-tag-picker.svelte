...
<Popover.Root bind:open>
  <Popover.Trigger>
    {#snippet child({ props })}
      <Button {...props} type="button" variant="outline" size="icon" aria-label={$t('docs.tags.add')}>
        <PlusIcon size={16} />
      </Button>
    {/snippet}
  </Popover.Trigger>
  <Popover.Content align="start" class="w-[420px] p-3">
    <div class="space-y-4">
      <p class="text-sm font-semibold">{$t('docs.tags.popover_title')}</p>
      {#if !tagGroups.length}
        <p class="ui:text-muted-foreground text-sm">{$t('docs.tags.empty_groups')}</p>
      {:else}
