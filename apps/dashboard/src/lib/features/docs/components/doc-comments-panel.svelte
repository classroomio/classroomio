<script lang="ts">
  ...
  const openThreads = $derived(docCommentsApi.threads.filter((thread) => thread.status === 'open'));
  const resolvedThreads = $derived(docCommentsApi.threads.filter((thread) => thread.status === 'resolved'));
  ...
  $effect(() => {
    ...
    const thread = docCommentsApi.threads.find((item) => item.id === activeThreadId);
  ...
  $effect.pre(() => {
    for (const thread of docCommentsApi.threads) {
  ...
  function mentionTypeLabel(_item: MentionItem) {
    return t.get('docs.comments.mention_team_member');
  }
  ...
  async function submitReply(threadId: string) {
    ...
    await docCommentsApi.addReply(docId, threadId, body);
  ...
  async function saveEdit(commentId: string) {
    ...
    await docCommentsApi.updateComment(docId, commentId, body);
    editSubmitting[commentId] = false;
  }

  async function deleteComment(commentId: string) {
    await docCommentsApi.deleteComment(docId, commentId);
  }
...
>
  <header class={cn('px-4 py-3', embedded ? '' : 'ui:border-border border-b')}>
    ...
    <div class={cn('flex gap-1', embedded ? '' : 'mt-3')}>
      <Button size="sm" variant={activeTab === 'open' ? 'default' : 'secondary'} onclick={() => (activeTab = 'open')}>
        {$t('docs.comments.open_heading')} ({openThreads.length})
      </Button>
      <Button
        size="sm"
      ...
      >
        {$t('docs.comments.resolved_heading')} ({resolvedThreads.length})
      </Button>
    </div>
  </header>
  ...
  <div class="min-h-0 flex-1 space-y-4 overflow-y-auto p-3">
    {#if pendingComposer && activeTab === 'open'}
      <article class="ui:bg-card ui:border-border rounded-lg border p-3 ring-2 ring-amber-400">
        ...
        <label class="mb-1 block text-xs font-medium" for="pending-comment"
          >{$t('docs.comments.new_thread_label')}</label
        >
        <ChatTextarea
          bind:value={pendingComposer.draft}
          {mentionItems}
          typeLabel={mentionTypeLabel}
          placeholder={$t('docs.comments.reply_placeholder')}
          emptyMessage={$t('docs.comments.mention_no_results')}
          rows={2}
        />
        <p class="ui:text-muted-foreground mt-2 text-xs">{$t('docs.comments.autosave_hint')}</p>
    ...
    {#if !pendingComposer && docCommentsApi.threads.length === 0}
      <p class="ui:text-muted-foreground px-1 text-sm">{$t('docs.comments.empty')}</p>
    {:else if visibleThreads.length === 0}
      <p class="ui:text-muted-foreground px-1 text-sm">
        {activeTab === 'open' ? $t('docs.comments.empty_open') : $t('docs.comments.empty_resolved')}
      </p>
    {/if}
              ...
              <div class="min-w-0 flex-1">
                <div class="flex items-center gap-2">
                  <p class="truncate text-xs font-medium">
                    {isAiAuthor(comment)
                      ? $t('docs.comments.ai_author')
                      : (comment.authorFullname ?? $t('docs.comments.anonymous'))}
                  </p>
                  {#if isAiAuthor(comment)}
                    <span class="ui:bg-muted ui:text-muted-foreground rounded px-1.5 py-0.5 text-[10px] font-medium">
                      {$t('docs.comments.ai_badge')}
                    </span>
                  {/if}
                ...
                {:else}
                  <button type="button" class="w-full text-left" onclick={() => handleSelectThread(thread)}>
                    <p class="doc-comment-body mt-1 text-sm whitespace-pre-wrap">
        ...
        {#if canComment && thread.status === 'open'}
          <div class="mt-3 space-y-2">
            <label class="block text-xs font-medium" for={`reply-${thread.id}`}
              >{$t('docs.comments.reply_label')}</label
            >
            <ChatTextarea
...
<style>
  :global(.doc-comment-body .doc-comment-mention) {
    font-weight: 600;
    color: var(--primary);
  }
</style>
