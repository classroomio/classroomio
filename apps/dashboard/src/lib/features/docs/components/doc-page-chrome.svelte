<script lang="ts">
  ...
  import NoteCoverUpload from './doc-cover-upload.svelte';
  ...
  const visibilityLabel = $derived(
    visibility === 'team'
      ? t.get('docs.page.visibility_team')
      : visibility === 'public'
        ? t.get('docs.page.visibility_public')
        : t.get('docs.page.visibility_private')
  );
...
<article class={cn('flex min-h-full w-full flex-col', className)}>
  <div class="group/cover relative z-0 w-full shrink-0">
    ...
    {#if canWrite}
      ...
      >
        ...
        >
          <ImageIcon size={14} />
          {coverImageUrl ? $t('docs.page.change_cover') : $t('docs.page.add_cover')}
        </Button>
  ...
  <div class="relative z-10 mx-auto flex w-full max-w-3xl flex-1 flex-col px-8 pb-16 md:px-12">
    ...
    <textarea
      ...
      placeholder={$t('docs.org.new_note_title')}
