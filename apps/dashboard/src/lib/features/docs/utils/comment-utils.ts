...
export function buildCommentAnchor(editor: Editor, threadId: string): TDocCommentAnchor {
...
export function stripCommentMarkFromHtml(html: string, threadId: string): string {
  ...
  template.content.querySelectorAll(`span[data-doc-comment="${threadId}"]`).forEach((element) => {
...
export function syncActiveCommentMark(root: HTMLElement | undefined, activeThreadId: string | null) {
  ...
  root.querySelectorAll('.doc-comment-mark').forEach((element) => {
    element.classList.remove('doc-comment-mark-active');
  });
  ...
  const mark = root.querySelector(`span[data-doc-comment="${activeThreadId}"]`);

  if (mark) {
    mark.classList.add('doc-comment-mark-active');
  }
}

export function scrollToCommentAnchor(root: HTMLElement, anchor: TDocCommentAnchor, activeThreadId?: string | null) {
  const mark = root.querySelector(`span[data-doc-comment="${anchor.threadId}"]`);
  ...
  pulse.className = 'doc-comment-anchor-pulse';
...
export function reapplyCommentMarkInEditor(editor: Editor, threadId: string, anchor: TDocCommentAnchor): boolean {
  if (editor.getHTML().includes(`data-doc-comment="${threadId}"`)) {
    return false;
  }
