...
import { buildNoteTree, partitionSidebarSections } from './doc-tree-utils';
import type { SidebarDocItem } from '../api/docs.svelte';

function makeNote(overrides: Partial<SidebarDocItem> = {}): SidebarDocItem {
  return {
    id: 'note-1',
...
describe('partitionSidebarSections', () => {
  ...
  it('routes team notes to workspace', () => {
    ...
    expect(sections.organization.map((note) => note.id)).toEqual(['team-1']);
