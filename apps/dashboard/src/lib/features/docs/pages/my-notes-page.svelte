<script lang="ts">
  ...
  import NoteTemplatesRow from '../components/doc-templates-row.svelte';
  ...
  const filteredNotes = $derived.by(() => {
    ...
    return docsApi.notes.filter((note) => {
  ...
  $effect(() => {
    ...
    docsApi.listNotes({ tagId, scope: listScope });
...
{#snippet noteCard(note: DocListItem)}
    ...
    <Card.Header class="gap-2 pb-0">
      ...
      {#if note.ownerId !== $profile.id && note.ownerFullname}
        <Card.Description class="text-xs">
          {$t('docs.list.by_author', { name: note.ownerFullname })}
        </Card.Description>
      {/if}
    </Card.Header>
...
<div class="flex flex-col gap-4">
  {#if showWorkspaceUsage && docsApi.usage?.limit !== null && docsApi.usage}
  ...
  <UnderlineTabs.Root bind:value={currentTab}>
    <UnderlineTabs.List>
      <UnderlineTabs.Trigger value="all">{$t('docs.list.tabs.all')}</UnderlineTabs.Trigger>
      {#if showTeamTab}
        <UnderlineTabs.Trigger value="team">{$t('docs.list.tabs.team')}</UnderlineTabs.Trigger>
      {/if}
      <UnderlineTabs.Trigger value="lessons">{$t('docs.list.tabs.from_lessons')}</UnderlineTabs.Trigger>
    </UnderlineTabs.List>
  </UnderlineTabs.Root>
  ...
  {:else if filteredNotes.length === 0}
    <Empty title={$t('docs.list.empty_title')} description={$t('docs.list.empty_description')} />
  {:else}
