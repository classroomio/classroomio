<script lang="ts">
  ...
  interface Props {
    /** When set, only show goals from this program. */
    programId?: string;
  }
  ...
  onMount(() => {
    programGoalApi.listMyGoals();
  });

  const myGoals = $derived(
    programGoalApi.myGoals.filter((assignment) => !programId || assignment.programId === programId)
  );

  function formatDue(assignment: (typeof programGoalApi.myGoals)[number]): string {
    if (!assignment.dueDate) return $t('programs.goals.lms.no_deadline');

    const dueDate = new Date(assignment.dueDate);
    const now = new Date();
    const days = Math.ceil((dueDate.getTime() - now.getTime()) / (24 * 60 * 60 * 1000));

    if (days < 0) return $t('programs.goals.lms.overdue');
...
{#if myGoals.length > 0}
  <section class="mb-4">
    <h3 class="mb-2 text-sm font-semibold">{$t('programs.goals.lms.heading')}</h3>
        ...
        <div class="flex flex-col gap-2 rounded-md border p-3">
          <div class="flex flex-wrap items-center justify-between gap-2">
            <div class="min-w-0">
              ...
              {#if !programId}
                <p class="ui:text-muted-foreground text-xs">{assignment.programName}</p>
              {/if}
            </div>
            <Badge variant={statusVariant(assignment.status)}>
              {$t(`programs.goals.status.${assignment.status}`)}
            </Badge>
          </div>
