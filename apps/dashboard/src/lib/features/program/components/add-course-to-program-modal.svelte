<script lang="ts">
  ...
  let { open = $bindable(false), programId }: Props = $props();
  ...
  const emptyMessage = $derived(
    searchValue.trim()
      ? $t('programs.courses.no_matching_courses') || 'No courses match your search.'
      : $t('programs.courses.no_available_courses') || 'No available courses to add.'
  );
...
<Dialog.Root bind:open>
  <Dialog.Content class="max-w-md">
    <Dialog.Header>
      <Dialog.Title>{$t('programs.courses.add_modal_title') || 'Add Course to Program'}</Dialog.Title>
    </Dialog.Header>
    ...
    <div class="flex flex-col gap-4 py-2">
      <MultiSelectList
        ...
        listClass="max-h-60"
        searchPlaceholder={$t('programs.courses.search_placeholder') || 'Search courses'}
        bind:searchValue
      >
        {#snippet headingSnippet()}
          <p class="ui:text-sm ui:font-medium">
            {$t('programs.courses.select_courses') || $t('programs.courses.select_course') || 'Select course(s)'}
            {#if selectedCount > 0}
      ...
      {#if totalPages > 1}
        <div class="flex items-center justify-between gap-2 text-sm">
          <span class="ui:text-muted-foreground">
            {$t('programs.courses.pagination_status', {
              page: currentPage,
              totalPages
    ...
    <Dialog.Footer>
      <Button variant="outline" onclick={closeModal} disabled={programApi.isLoading}>
        {$t('app.cancel') || 'Cancel'}
      </Button>
      <Button onclick={handleAdd} loading={programApi.isLoading} disabled={selectedCount === 0 || programApi.isLoading}>
