      ...
      <DropdownMenu.Root>
        ...
        <DropdownMenu.Content align="end">
          <DropdownMenu.Item onclick={() => onPin(feed.id, feed.isPinned)}>
            {feed.isPinned ? $t('programs.newsfeed.card.unpin') : $t('programs.newsfeed.card.pin')}
          </DropdownMenu.Item>
          <DropdownMenu.Item onclick={onEdit}>{$t('programs.newsfeed.card.edit')}</DropdownMenu.Item>
          <DropdownMenu.Item class="text-red-600" onclick={onRequestDelete}>
            {$t('programs.newsfeed.card.delete')}
          </DropdownMenu.Item>
