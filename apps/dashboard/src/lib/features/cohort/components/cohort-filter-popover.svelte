<script lang="ts">
  ...
  interface Props {
    sortKey?: CohortSortBy;
    selectedOrder?: CohortSortOrder;
  ...
  let {
    sortKey = $bindable(DEFAULT_COHORT_SORT),
    selectedOrder = $bindable(DEFAULT_COHORT_SORT_ORDER),
    selectedStatuses = [],
    statusOptions = [],
    sortOptions = [],
    onToggleStatus = () => {},
  ...
  const hasActiveFilters = $derived(
    sortKey !== DEFAULT_COHORT_SORT || selectedOrder !== DEFAULT_COHORT_SORT_ORDER || selectedStatuses.length > 0
  );
...
<Popover.Root bind:open={isOpen}>
  <Popover.Trigger>
    {#snippet child({ props })}
      <div class="relative">
        <IconButton {...props} aria-label={$t('cohorts.filters.filter')} tooltip={$t('cohorts.filters.filter')}>
          <FilterIcon size={16} />
        </IconButton>
        {#if hasActiveFilters}
          <span
  ...
  <Popover.Content align="end" class="w-[360px] p-3">
    <div class="space-y-4">
      ...
      <div class="space-y-2">
        ...
        >
          <ToggleGroup.Item value="asc">{$t('cohorts.filters.ascending')}</ToggleGroup.Item>
          <ToggleGroup.Item value="desc">{$t('cohorts.filters.descending')}</ToggleGroup.Item>
        </ToggleGroup.Root>
      </div>

      <div class="space-y-3">
        <p class="ui:text-muted-foreground text-xs font-semibold uppercase">
          {$t('cohorts.filters.status')}
        </p>
        <div class="space-y-2">
          {#each statusOptions as option (option.value)}
            <label class="ui:hover:bg-muted/30 flex cursor-pointer items-center rounded-md border px-3 py-2">
