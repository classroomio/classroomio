<script lang="ts">
  ...
  interface Props {
    ...
    canComment?: boolean;
    comments?: CohortNewsfeedCommentsByFeedId;
    onAddComment: (content: string) => Promise<void> | void;
    onDeleteComment: (commentId: string | number) => void;
  }

  let { cohortId, feed, author, canComment = true, comments, onAddComment, onDeleteComment }: Props = $props();
  ...
  const bootstrap = async () => {
    ...
    try {
      await cohortNewsfeedApi.getComments(cohortId, feed.id);
    } finally {
...
<section class="border-t border-gray-200 p-3">
  <div class="flex w-full items-center justify-between">
    ...
    >
      <UsersIcon size={16} />
      <p class="py-2 text-sm">{pluralize($t('cohorts.newsfeed.comments.comment'), totalCount, true)}</p>

      {#if totalCount > shownCount}
        <span class="ml-auto text-xs text-gray-600">
          {$t('cohorts.newsfeed.comments.showing_of', { shown: shownCount, total: totalCount })}
        </span>
      {/if}
    </Button>
    ...
    {#if shownCount > 1}
      <Button variant="ghost" size="sm" onclick={minimize}>
        <span class="ml-auto text-xs text-gray-600">{$t('cohorts.newsfeed.comments.hide')}</span>
      </Button>
    {/if}
  </div>
  ...
  {#if canComment}
    <div class="flex items-center justify-between gap-2">
      <UserAvatar src={author.avatarUrl} alt={$t('cohorts.newsfeed.user_avatar_alt')} class="size-7" />
      <div class="flex-1">
        <input
          type="text"
          bind:value={comment}
          onkeydown={handleAddNewComment}
          placeholder={$t('cohorts.newsfeed.comments.placeholder')}
      ...
      <Button
        ...
        aria-label={$t('cohorts.newsfeed.comments.submit_aria')}
      >
