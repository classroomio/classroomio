<script lang="ts">
  ...
  let { cohortId, canManage }: Props = $props();

  let isModalOpen = $state(false);
  let editingGoal = $state<CohortGoal | null>(null);

  const goals = $derived(cohortGoalApi.goalsFor(cohortId));

  onMount(() => {
    cohortGoalApi.listGoals(cohortId);
  });
  ...
  function openEdit(goal: CohortGoal) {
    editingGoal = goal;
    isModalOpen = true;
  }

  async function deleteGoal(goal: CohortGoal) {
    const confirmMessage = $t('cohorts.goals.delete_confirm');
    if (!confirm(confirmMessage)) return;

    await cohortGoalApi.deleteGoal(cohortId, goal.id);
  }

  function summarizeDeadline(goal: CohortGoal) {
    if (goal.deadlineKind === 'none') return $t('cohorts.goals.lms.no_deadline');
    ...
    return $t('cohorts.goals.lms.no_deadline');
  }

  function totalLearners(goal: CohortGoal): number {
    return Object.values(goal.statusCounts ?? {}).reduce((sum, value) => sum + (value ?? 0), 0);
  }

  function statusValue(goal: CohortGoal, status: string): number {
...
<section>
  <div class="mb-3 flex items-start justify-between gap-3">
    <div>
      <h2 class="text-sm font-semibold">{$t('cohorts.goals.heading')}</h2>
      <p class="ui:text-muted-foreground mt-1 text-xs">{$t('cohorts.goals.description')}</p>
    </div>
    {#if canManage}
  ...
  {#if goals.length === 0}
    <div class="rounded-md border p-6 text-center">
      <p class="text-sm font-medium">{$t('cohorts.goals.empty_title')}</p>
      <p class="ui:text-muted-foreground mt-1 text-xs">{$t('cohorts.goals.empty_description')}</p>
    </div>
  {:else}
        ...
        <div class="flex flex-col gap-3 rounded-md border p-4">
          ...
          <div class="grid grid-cols-2 gap-2 sm:grid-cols-4">
            <div class="rounded border p-2 text-center">
              <p class="ui:text-muted-foreground text-xs">{$t('cohorts.goals.status.completed')}</p>
              <p class="text-base font-semibold">{completed}/{total}</p>
            </div>
            <div class="rounded border p-2 text-center">
              <p class="ui:text-muted-foreground text-xs">{$t('cohorts.goals.status.in_progress')}</p>
            ...
            <div class="rounded border p-2 text-center {atRisk > 0 ? 'bg-amber-50' : ''}">
              <p class="ui:text-muted-foreground text-xs">{$t('cohorts.goals.status.at_risk')}</p>
              <p class="text-base font-semibold">{atRisk}</p>
            </div>
            <div class="rounded border p-2 text-center {overdue > 0 ? 'bg-red-50' : ''}">
              <p class="ui:text-muted-foreground text-xs">{$t('cohorts.goals.status.overdue')}</p>
...
<GoalFormModal bind:open={isModalOpen} {cohortId} goal={editingGoal} onClose={() => (isModalOpen = false)} />
