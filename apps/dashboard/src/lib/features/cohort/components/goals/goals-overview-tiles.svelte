<script lang="ts">
  ...
  function onTrackPct(goal: CohortGoal): number {
...
{#if goals.length > 0}
  <section class="mb-4">
    <h3 class="mb-2 text-sm font-semibold">{$t('cohorts.goals.heading')}</h3>
      ...
      {#each goals as goal (goal.id)}
        ...
        <div class="flex flex-col gap-2 rounded-md border p-3">
          ...
          <div class="flex items-baseline gap-1">
            ...
            <span class="ui:text-muted-foreground text-xs">{$t('cohorts.goals.tile.on_track')}</span>
          </div>
          <div class="flex flex-wrap gap-3 text-xs">
            <span>
              <span class="font-medium">{completed}/{total}</span>
              {' '}{$t('cohorts.goals.tile.completed_short')}
            </span>
            {#if atRisk > 0}
