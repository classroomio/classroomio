<script lang="ts">
  ...
  import NoteSubpages from '$features/docs/components/doc-subpages.svelte';
  import { displayNoteTitle } from '$features/docs/utils/doc-list-utils';
  ...
  const docTitle = $derived(displayNoteTitle(note.title));
  ...
  const hrefForSlug = (slug: string) => `/doc/${slug}`;
...
>
  <article class="mx-auto flex w-full max-w-3xl flex-1 flex-col gap-6 px-4 py-10 sm:px-6">
    <header class="space-y-2 text-center">
      ...
      <h1 class="text-3xl font-bold tracking-tight">{docTitle}</h1>
