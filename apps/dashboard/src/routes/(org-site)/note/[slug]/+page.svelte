<script lang="ts">
  ...
  import type { PublicCourseOrgData, PublicCourseSidebarSection } from '@cio/ui/custom/public-course';
  ...
  const breadcrumbs = $derived(
    note.breadcrumbs.map((crumb, index) => ({
      label: displayNoteTitle(crumb.title),
      href: index < note.breadcrumbs.length - 1 ? hrefForSlug(crumb.slug) : undefined
    }))
  );

  const sections = $derived<PublicCourseSidebarSection[]>([
    {
      id: 'outline',
      title: displayNoteTitle(note.rootTitle),
      items: note.outline.map((item) => ({
        kind: 'lesson' as const,
        id: item.id,
        slug: item.slug,
        title: displayNoteTitle(item.title),
        isUnlocked: true,
        depth: item.depth
      }))
    }
...
<PublicCourse.PublicCourseShell
  {sections}
  courseTitle={noteTitle}
  {breadcrumbs}
  {org}
  activeSlug={note.slug}
  hrefFor={(item) => hrefForSlug(item.slug)}
  homeHref="/"
  exploreHref="/"
  ...
  orgSlug={org.siteName ?? org.name}
>
  <article class="mx-auto flex w-full max-w-3xl flex-col gap-6 px-4 py-10 sm:px-6">
