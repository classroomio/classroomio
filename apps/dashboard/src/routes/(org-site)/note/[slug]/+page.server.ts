...
type GetPublicNoteRequest = (typeof classroomio)['org-site']['note'][':noteSlug']['$get'];
type GetPublicNoteSuccess = Extract<InferResponseType<GetPublicNoteRequest>, { success: true }>;
...
export const load = async ({ params, parent, url }) => {
  ...
  const noteResult = await safeServerApi<GetPublicNoteSuccess>(() =>
    classroomio['org-site'].note[':noteSlug'].$get(
      {
        param: { noteSlug: params.slug },
        query: { siteName }
      },
      getApiKeyHeaders()
    )
