<script lang="ts">
  ...
  const rootLoader = $derived(plugin?.routes?.['/'] ?? null);
...
{:else if rootLoader}
  ...
  {:then { default: PluginPage }}
    <PluginPage {orgSlug} />
  {:catch}
    <div class="flex h-full items-center justify-center text-sm text-red-500">{$t('plugins.load_error')}</div>
  {/await}
{:else}
  <div class="flex h-full items-center justify-center text-sm text-slate-400">{$t('plugins.empty')}</div>
{/if}
