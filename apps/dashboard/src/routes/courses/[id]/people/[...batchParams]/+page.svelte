...
<CourseContainer bind:courseId={data.courseId}>
  <PageNav title={$t('course.navItem.people.title')} disableSticky={true}>
    <slot:fragment slot="widget">
      <div class="flex items-center gap-x-4">
        ...
        <RoleBasedSecurity allowedRoles={[1, 2]}>
          <button on:click={() => openBatchSettings()}>
            <Settings size={32} />
          </button>
        </RoleBasedSecurity>
      </div>
  ...
  <PageBody width="w-full max-w-6xl md:w-11/12">
    <section class="my-5 mx-2 md:mx-9">
      <div
        class="flex flex-col md:flex-row flex-end gap-2 justify-between items-start md:items-center mb-7"
      >
        <button on:click={() => goBackToBatches()} class="flex items-center cursor-pointer gap-x-2">
          <ArrowLeft size={24} />
          <p class="text-xl font-medium">[TODO: Cohort name]</p>
        </button>
