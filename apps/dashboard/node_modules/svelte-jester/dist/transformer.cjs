...
const isSvelte3 = (version = compiler$1.VERSION) => version.startsWith('3');
...
const processAsync = async (source, filename, jestOptions) => {
  ...
  const processed = await compiler$1.preprocess(
    source,
...
const compiler = (format, options = {}, filename, processedCode, processedMap) => {
  ...
  let result;
  try {
    result = compiler$1.compile(processedCode, opts);
  } catch (error) {
