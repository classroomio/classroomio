...
export const mailRouter = new Hono().post('/send', zValidator('json', ZSendEmailValidation), async (c) => {
  const validatedData = c.req.valid('json');

  const results = await Promise.all(
    validatedData.map(async (emailData) => {
      try {
        const res = env.ZOHO_TOKEN ? await sendWithZoho(emailData) : await sendWithNodemailer(emailData);
