...
import { getDocById, updateDoc } from '@cio/db/queries/docs';
import { getNoteSharePermission } from '@cio/db/queries/docs/share';
import type { TConvertNoteToCourse } from '@cio/utils/validation/docs';
import { isOrgTeamRole, resolveNoteAccess } from './access';

async function getWritableWorkspaceNote(organizationId: string, userId: string, roleId: number, docId: string) {
...
export async function convertNoteToCourseService(
  ...
  const note = await getWritableWorkspaceNote(organizationId, userId, roleId, docId);
  ...
  if (note.convertedCourseId) {
    throw new AppError('This note was already converted to a course', ErrorCodes.DOC_ALREADY_CONVERTED, 409);
  }
