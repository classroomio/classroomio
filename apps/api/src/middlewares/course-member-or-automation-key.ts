...
import { courseMemberMiddleware } from './course-member';

export const courseMemberOrAutomationKeyMiddleware =
  ...
  async (c: Context, next: Next) => {
    try {
      ...
      if (!automationKey) {
        return courseMemberMiddleware(c, async () => {
      ...
      console.error('Error in courseMemberOrAutomationKeyMiddleware:', error);
      return c.json(
        {
          success: false,
