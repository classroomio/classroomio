...
<div transition:fly={{ y: 100, delay: 0, easing: sineInOut }}>
  <div class="px-6 pt-8 text-center">
    ...
    <button
      class="border mx-auto mt-7 mb-3 px-2 py-0.5 rounded-full flex items-center gap-1 text-[11px] text-[#656565] font-semibold border-[#C2D2FF] bg-[#DCE5FF]"
    >
      {activityName}:
  ...
  <div class="flex">
    ...
    >
      {#if isPaused}
        <img src="/free-tools/timer/play-icon.svg" alt="" class="w-7 md:w-12" />
      {:else}
        <img src="/free-tools/timer/pause-icon.svg" alt="" class="w-7 md:w-12" />
      {/if}
    ...
    >
      <img src="/free-tools/timer/reset-icon.svg" alt="" class="w-7 md:w-12" />
      restart</button
    >
  </div>
</div>
