...
import { Callout, codeode } from 'nextra/components';
...
<Callout type="info" emoji="ℹ️">
  Supabase schema is not created automatically during deployment. Apply the schema to your Supabase
  project before or during deployment. 
  
  Possible approaches: 
  
  - Supabase CLI (recommended before deploy):
  
  Open your local development environment and run this command.
   <code>
   ```bash 
      supabase login 
...
- Supabase Studio: Open SQL Editor and run the SQL from <code>`supabase/migrations/ `</code>
  in order

- CI/CD: Add a step that runs.
...
To enable deployment using docker, we provide a `docker-compose.yaml` configuration file to deploy the dashboard and backend services.
...
2. **Create Environment File**:
   You can manually create an .env file yourself or use the command below to quickly create both .env files needed.
...
<Callout type="info">This section is a work in progress.</Callout>
