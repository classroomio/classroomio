...
Cohort tools call the public API and need the key's `public_api:*` scope. They act as the person who created the key, with the same permissions that person has in the dashboard:
