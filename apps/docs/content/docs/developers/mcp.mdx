...
Cohort tools call the public API and need the key's `cohort:read` and `cohort:write` scopes, which MCP keys have by default. MCP keys can't reach the rest of the public API. They act as the person who created the key, with the same permissions that person has in the dashboard:
