...
**Docker with MinIO:** Use the `minio` profile to run MinIO locally. Add to your `.env`:
...
# API storage config (use minio:9000 for Docker internal, localhost:9000 for browser)
...
Then start the stack with the minio profile:

```zsh title="Terminal"
docker compose -p classroomio -f docker/docker-compose.yaml --profile minio up -d
```

This starts MinIO on port 9000 and creates `videos`, `documents`, and `media` buckets.
