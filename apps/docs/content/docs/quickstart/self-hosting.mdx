...
We use Cloudflare R2 for video uploading within the course builder. Configure these env vars:
...
### Option B: MinIO (S3-compatible, On-Premise)

For on-premise deployments or data residency requirements, use MinIO or any S3-compatible storage:
...
**Docker with MinIO:** Use the `minio` profile to run MinIO locally:

```zsh title="Terminal"
docker compose -p classroomio -f docker/docker-compose.yaml --profile minio up -d
```

This starts MinIO on port 9000 and creates `videos`, `documents`, and `media` buckets. Set the `OBJECT_STORAGE_*` env vars above in your `.env` to use it. When the API runs inside Docker, use `OBJECT_STORAGE_ENDPOINT=http://minio:9000` and `OBJECT_STORAGE_PUBLIC_ENDPOINT=http://localhost:9000` so the browser can reach MinIO.

<Callout title="Storage Optional">
  Object storage is not required to run the app. You can skip it if you don't need video/document uploads.
</Callout>
