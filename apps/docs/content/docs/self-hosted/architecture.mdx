...
Every install path (Docker, DigitalOcean, Railway, Coolify, Dokploy) runs the same topology, so
this page applies no matter which one you picked.
...
([details](/self-hosted/troubleshooting#uploads-stuck-at-processing)). It also scales
independently: the image runs all workers in one process by default, but you can override the
container command to run, say, media processing on separate hardware from email sending.
