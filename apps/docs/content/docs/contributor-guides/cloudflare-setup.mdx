...
In this guide, you will learn how to get your Cloudflare environment variables and configure R2 buckets. ClassroomIO supports Cloudflare R2 for video uploads, document uploads, and media (images and video thumbnails). For on-premise or data-residency deployments, you can use [MinIO](/docs/self-hosted/docker#optional--object-storage) instead.
...
**Upload size limits** (self-hosted / on-prem): optional `UPLOAD_MAX_*_MB` env vars cap documents, videos, assignment files, and other uploads. Set the same values on the API and dashboard. See [Docker self-hosting → Upload file size limits](/docs/self-hosted/docker#optional--upload-file-size-limits). If you raise these, also increase any reverse-proxy body-size limit in front of your API or dashboard.
