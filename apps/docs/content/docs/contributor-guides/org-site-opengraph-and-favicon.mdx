...
For admin-facing behavior, see [Sharing Your Academy (Open Graph & Favicon)](/docs/academy-sharing-and-branding).
...
2. **Cloud CDN** — `PUBLIC_MEDIA_CDN_URL` or default `https://img.cdn.clsrio.com` → `/og/{siteName}.png`
3. `PUBLIC_SERVER_URL` — direct API URL `/org-site/og/{siteName}.png`
...
1. Custom `org.favicon` if set (uploaded in **Settings → Domains**)
...
<Callout type="info" title="No save-hook regen">
...
- [Sharing Your Academy (Open Graph & Favicon)](/docs/academy-sharing-and-branding) — admin-facing guide
- [Cloudflare Setup](/docs/contributor-guides/cloudflare-setup) — R2 media bucket and `CLOUDFLARE_IMAGE_BUCKET_DOMAIN`
- [Customize Your Organization](/docs/customize-organization) — logo, name, theme (user docs)
