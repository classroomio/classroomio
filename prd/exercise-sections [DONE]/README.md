# Exercise Question Sections PRD
...
Allow instructors to group questions inside a single exercise into named sections, so one assignment, quiz, or exam can be structured into logical parts (e.g., "Section A: Multiple Choice", "Section B: Written Responses") with per-section instructions, navigation, and review.

## Terminology

- **Exercise question section**: A grouping inside one exercise. This PRD is about this concept.
- **Course section**: The existing course content grouping that contains lessons and exercises. This PRD does not change course sections.
- The existing `exercise.sectionId` field points to a **course section**. New question-level grouping must use `question.exerciseSectionId` / DB `question.exercise_section_id` so the two concepts are not overloaded.
...
| --- | --- | --- |
| `exercise` table | Has `title`, `description`, and `section_id` for the parent course section; questions link via `question.exercise_id` | No internal question-section concept |
| `question` table | Has `exercise_id`, `order`, `title`, `points`, `question_type_id` | No `exercise_section_id` or section ordering |
...
5. Let instructors assign lightweight visual color themes to sections for easier scanning while authoring.
6. Maintain full backward compatibility — existing sectionless exercises work unchanged.
7. Support sections in exercise templates and cloning.
...
- Custom arbitrary color pickers. V1 uses a fixed set of color theme swatches.

## Proposed Decisions / MVP Assumptions
...
2. A question belongs to exactly **one exercise question section**. No multi-section membership.
...
13. Section color themes are cosmetic only. They help authoring and review scanability and do not affect grading, routing, or student eligibility.
...
- Instructor can choose a section **color theme** from a fixed set of swatches. The selected theme appears as a dot in the section header and is persisted with the section.
- Instructor can configure the **after-section behavior** via a compact settings row near the section card when routing is in scope:
...
- Section ordering, titles, descriptions, color themes, and after-behavior settings are preserved.
...
CREATE TABLE exercise_section (
  ...
  color_theme VARCHAR NOT NULL DEFAULT 'blue',
...
{ action: 'go_to_section', exerciseSectionId: '<uuid>' }

// Submit the exercise immediately
{ action: 'submit' }
```

#### 1.2: `question` Table — Add `exercise_section_id`

```sql
ALTER TABLE question ADD COLUMN exercise_section_id UUID REFERENCES exercise_section(id) ON DELETE SET NULL;
CREATE INDEX idx_question_exercise_section_id ON question(exercise_section_id);
```

- `exercise_section_id` is **nullable**. When `NULL`, the question belongs to a sectionless exercise (backward compatible).
- When a question is part of a sectioned exercise, `exercise_section_id` is required (enforced at the application level, not DB constraint, to keep migration safe).
...
export interface ExerciseSectionAfterBehavior {
  ...
  exerciseSectionId?: string; // only when action === 'go_to_section'
}
...
| `updateExerciseSection(exerciseSectionId, data)` | Update a single section's title, description, colorTheme, order, or afterBehavior |
| `deleteExerciseSection(exerciseSectionId)` | Delete a section (questions cascade to `exercise_section_id = NULL`) |
| `reorderExerciseSections(exerciseId, orderedIds[])` | Batch update `order` field for all sections |
| `moveQuestionToSection(questionId, exerciseSectionId)` | Update a question's `exercise_section_id` |
| `bulkMoveQuestionsToSection(questionIds[], exerciseSectionId)` | Move multiple questions to a section |
...
```typescript
{
  ...
  sections: Array<{
    ...
    afterBehavior: { action: string; exerciseSectionId?: string };
...
2. Update `question.exercise_section_id` for each question based on `questionIds` in each section.
...
4. When cloning questions, include the mapped `exerciseSectionId`.
...
if (oldSections.length > 0) {
  ...
  oldSections.forEach((oldSection, index) => {
    exerciseSectionIdMap.set(oldSection.id, newSections[index].id);
  });

  // When cloning questions, include mapped exerciseSectionId:
  // exerciseSectionId: question.exerciseSectionId ? exerciseSectionIdMap.get(question.exerciseSectionId) : null
}
```

Update `afterBehavior` references: if a section's `afterBehavior.action === 'go_to_section'`, remap the `exerciseSectionId` using the section ID map.
...
Add `exerciseSectionId` to the `Question` type:
...
export function handleRemoveSection(exerciseSectionId: string, moveQuestionsToExerciseSectionId?: string) {
  questionnaire.update((q) => {
    const section = q.sections.find((s) => s.id === exerciseSectionId);
    if (!section) return q;

    let updatedQuestions = q.questions;
    if (moveQuestionsToExerciseSectionId) {
      updatedQuestions = q.questions.map((question) =>
        question.exerciseSectionId === exerciseSectionId
          ? { ...question, exerciseSectionId: moveQuestionsToExerciseSectionId, isDirty: true }
          : question
      );
    } else {
      updatedQuestions = q.questions.map((question) =>
    ...
    return {
      ...
      sections: q.sections.map((s) =>
        s.id === exerciseSectionId ? { ...s, deletedAt: new Date().toString() } : s
      ),
...
export function handleMoveQuestionToSection(questionId: string, exerciseSectionId: string) {
  questionnaire.update((q) => ({
    ...q,
    questions: q.questions.map((question) =>
      question.id === questionId
        ? { ...question, exerciseSectionId, isDirty: true }
        : question
    )
  }));
}
...
export function getQuestionsForSection(
  ...
  return questions
    .filter((q) => q.exerciseSectionId === exerciseSectionId && !q.deletedAt)
...
#### 8.1: Edit Mode — Section Cards

File: `apps/dashboard/src/lib/features/course/components/exercise/edit-mode.svelte`

When sections exist, render questions inside section cards with three distinct zones:

```
┌────────────────────────────────────────────────────────┐
│ ⋮⋮  ● [Section title input              ] [3 questions] × │  Header
│     ● ● ● ● ●                                                │  Color swatches
├────────────────────────────────────────────────────────┤
│  ┌─ Question sub-card ──────────────────────────────┐  │
│  │ Q1: What is...                         [Radio ▼] │  │
│  └──────────────────────────────────────────────────┘  │
│  ┌─ Question sub-card ──────────────────────────────┐  │  Content
│  │ Q2: Explain...                       [Textarea ▼]│  │
│  └──────────────────────────────────────────────────┘  │
│                         [ + Add question ]             │
├────────────────────────────────────────────────────────┤
│  3 required questions                         [↑] [↓]  │  Footer
└────────────────────────────────────────────────────────┘
...
Header:
- Sits on a secondary background to visually separate it from the white content area.
- Contains, from left to right: drag handle, color theme dot, editable title input, color swatches, question count badge, and remove button.
- The title input should visually blend into the header background until focused.
- Color swatches update the section's `colorTheme`; the dot reflects the selected theme.

Content:
- Sits on the primary white background with comfortable padding.
- Renders question items stacked vertically.
- Each question item is its own bordered sub-card.
- A centered "Add question" button appears below the question list and appends a new question to that section.

Footer:
- Separated from content by a top border.
- Shows a small muted required-question count on the left. Until optional questions ship, this count equals the active question count.
- Shows small up/down arrow icon buttons on the right for moving the section within the exercise.

If post-section routing remains in v1, render the "After section N" control as a compact settings row above the footer or in section settings. Do not overload the footer, which is reserved for required count and section reorder controls.
If section descriptions remain in v1, expose description editing in an expandable section settings area rather than in the compact header.
Questions can be dragged between sections, with context-menu movement as a fallback.

#### 8.2: New Dashboard Component — `SectionEditor`

File: `apps/dashboard/src/lib/features/course/components/exercise/section-editor.svelte` (new)
...
interface Props {
  ...
  onColorThemeChange: (theme: ExerciseSectionColorTheme) => void;
  onAddQuestion: () => void;
  onDelete: () => void;
  onMoveUp: () => void;
  onMoveDown: () => void;
...
- Renders the three-zone edit-mode section card described above.
- Reuses existing question editor markup in the content zone.
- Uses existing UI primitives for buttons, inputs, badges, and icon buttons.
- Keep take/preview/read-only section headers separate from this editor component so student-facing screens do not inherit authoring controls.
...
A dropdown component that renders the "After section N" routing control. It should be visually adjacent to the section editor, but not inside the section editor footer.
...
```json
{
  ...
  "section.drag": "Drag section",
  "section.reorder": "Reorder Sections",
  "section.question_count": "{count} questions",
  "section.required_count": "{count} required questions",
  "section.color_theme": "Section color",
...
| Existing exercise, first section added | All questions auto-assigned to "Section 1". `exerciseSectionId` set on all questions. |
| Sectioned exercise, last section deleted | All questions become sectionless. `exerciseSectionId` set to `NULL`. Exercise reverts to flat mode. |
...
2. Add `exercise_section_id` column to `question` table (nullable, FK to `exercise_section`).
3. Add `section_display_mode` column to `exercise` table (default `'one_question'`).
4. Add `color_theme` to `exercise_section` with default `'blue'`.
5. Add index on `question.exercise_section_id`.
6. No data migration needed — existing exercises remain sectionless.
...
| `packages/ui/src/custom/exercise-question/section-header.svelte` | Read-only section header for take/preview/view modes |
...
| File | Changes |
| --- | --- |
| `packages/db/src/schema.ts` | Add `exerciseSection` table with `colorTheme`, `exerciseSectionId` to `question`, `sectionDisplayMode` to `exercise` |
...
| `apps/dashboard/src/lib/features/course/components/exercise/store.ts` | Add `sections`, `sectionDisplayMode`, `colorTheme`, section helpers |
...
| `apps/dashboard/src/lib/features/course/types.ts` | Extend `Question` with `exerciseSectionId` |
...
- Clone service preserves sections, question mappings, and remaps afterBehavior exerciseSectionIds.
...
| Clone service must remap `afterBehavior.exerciseSectionId` references | Explicit remap step in clone logic with validation |

---

## Open Questions

1. Should v1 include Google Forms-style post-section routing, or should v1 only support linear sections with submit at the end?
2. Should we support a "section shuffle" mode (randomize section order) independently of question shuffle?
3. Should the section navigation sidebar be collapsible or always visible during take mode?
4. Should after-behavior "Go to section X" carry over answered data, or should the target section reset?
