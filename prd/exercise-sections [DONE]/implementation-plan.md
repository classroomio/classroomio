# Exercise Question Sections — Implementation Plan

This plan implements sections inside a single exercise. It does not change course sections. The existing `exercise.sectionId` field continues to mean the course section that contains an exercise; the new question grouping uses `question.exerciseSectionId` / DB `question.exercise_section_id`.
...
3. Add `exerciseSectionId` column to the existing `question` table definition. Find the `question` table object and add this column after `exerciseId`:
...
2. The generated SQL should create the `exercise_section` table, add `exercise_section_id` to `question`, add `section_display_mode` to `exercise`, and create the index.
3. Review the generated migration carefully to ensure:
   - `exercise_section_id` on `question` is nullable (no `NOT NULL`).
   - `section_display_mode` on `exercise` has default `'one_question'`.
   - `color_theme` on `exercise_section` has default `'blue'`.
   - The `exercise_section` table has `ON DELETE CASCADE` for `exercise_id`.
   - The `question.exercise_section_id` FK has `ON DELETE SET NULL`.

**How to verify**:
- Migration runs successfully against a local database.
- Existing data is not affected.
- `SELECT * FROM exercise_section` returns empty.
- `SELECT exercise_section_id FROM question LIMIT 1` returns `NULL`.
...
export async function updateExerciseSection(
  ...
  return db
    ...
    .where(eq(exerciseSection.id, exerciseSectionId))
    .returning();
}

export async function deleteExerciseSection(exerciseSectionId: string) {
  return db.delete(exerciseSection).where(eq(exerciseSection.id, exerciseSectionId));
}

export async function moveQuestionToSection(questionId: number, exerciseSectionId: string | null) {
  return db
    .update(question)
...
export async function bulkMoveQuestionsToSection(questionIds: number[], exerciseSectionId: string | null) {
...
3. Group questions by `exerciseSectionId`:
   - Questions with an `exerciseSectionId` go into their respective section's `questions` array.
   - Questions without an `exerciseSectionId` go into the top-level `questions` array.
...
For each section in the payload:
  - If section has `id` and exists in DB → update title, description, colorTheme, order, afterBehavior
...
For each section:
  ...
  - Update each question's `exerciseSectionId` to this section's ID
...
4. Validation rules:
   ...
   - If any section has `afterBehavior.action === 'go_to_section'`, the referenced `exerciseSectionId` must exist in the same exercise.
...
let exerciseSectionIdMap = new Map<string, string>();
...
if (oldSections.length > 0) {
  ...
  // 4d. Remap afterBehavior.exerciseSectionId references
...
3. When inserting cloned questions, include `exerciseSectionId`:

```typescript
exerciseSectionId: question.exerciseSectionId ? exerciseSectionIdMap.get(question.exerciseSectionId) ?? null : null
```
...
- Verify cloned section color themes are preserved.
- Verify questions are assigned to correct cloned sections.
- Verify afterBehavior `exerciseSectionId` references point to the NEW section IDs (not old ones).
...
3. Add helper functions (see PRD Section 7.4):
   - `handleAddSection()`
   - `handleRemoveSection(exerciseSectionId, moveQuestionsToExerciseSectionId?)`
   - `handleMoveQuestionToSection(questionId, exerciseSectionId)`
   - `handleUpdateSectionAfterBehavior(exerciseSectionId, afterBehavior)`
   - `handleUpdateSectionColorTheme(exerciseSectionId, colorTheme)`
   - `getQuestionsForSection(questions, exerciseSectionId)`
   - `hasSections(sections)`
   - `handleReorderSections(orderedSectionIds)`

4. Update `handleAddQuestion()` to accept an optional `exerciseSectionId` parameter. When sections exist, the new question must have an `exerciseSectionId`.

5. Extend `Question` type in `apps/dashboard/src/lib/features/course/types.ts` to include `exerciseSectionId`.
...
- `handleMoveQuestionToSection()` updates the question's `exerciseSectionId`.
- `pnpm --filter @cio/dashboard build` succeeds.

---

### Ticket 3.2: Section Editor UI Component
...
1. Create `apps/dashboard/src/lib/features/course/components/exercise/section-editor.svelte`. This is the edit-mode section card that wraps existing question editor markup.
...
2. Render three distinct zones:
   - Header: secondary background, drag handle on the far left, selected color dot, editable title input that blends into the background, small color swatches, question count badge, and remove button.
   - Content: primary white background with padding, question items stacked vertically, each in a bordered sub-card, plus a centered "Add question" button below the list.
   - Footer: top border, muted required-question count on the left, small up/down arrow icon buttons on the right.

3. The color swatches use the supported `ExerciseSectionColorTheme` values. The swatch click updates the section state and marks it dirty.

4. Compute `requiredQuestionCount` from active questions where `required !== false`. Until optional questions are implemented, this equals the active question count.

5. Use icon buttons for drag/reorder/remove affordances. Use existing UI button/input primitives and `ui:` prefixes for UI theme color classes.

6. If section descriptions remain in v1, expose description editing in an expandable section settings area instead of placing it in the compact header.

**Files to create**:
- `apps/dashboard/src/lib/features/course/components/exercise/section-editor.svelte`

**Files to change**:
- `apps/dashboard/src/lib/features/course/components/exercise/edit-mode.svelte`

**How to verify**:
- Header, content, and footer zones render with the expected backgrounds and borders.
- Typing in the title fires the title callback.
- Selecting a color swatch updates the section color dot and store state.
- Clicking the centered "Add question" button creates a question assigned to the section.
- Up/down arrow buttons reorder sections and respect disabled states at the first/last section.
- `pnpm --filter @cio/dashboard build` succeeds.
...
2. Render a compact routing settings row near each section card, outside the section editor footer:
   - Label: "After section {N}" (left side).
   - A `<Select>` dropdown (right side) with options:
     - "Continue to next section" → `{ action: 'continue' }`
     - "Go to section: [Section Title]" → one entry per OTHER section → `{ action: 'go_to_section', exerciseSectionId: '...' }`
     - "Submit exercise" → `{ action: 'submit' }`
   - The currently selected option is shown in the trigger.

3. Style it as a secondary settings row. Do not place it in the section editor footer; that footer is reserved for required-question count and section reorder controls.
...
{#each activeSections as section, sectionIndex}
  <SectionEditor
    ...
    colorTheme={section.colorTheme}
    questionCount={getQuestionsForSection($questionnaire.questions, section.id).length}
    requiredQuestionCount={getRequiredQuestionCount(section.id)}
  ...
  >
    {#each getQuestionsForSection($questionnaire.questions, section.id) as question}
      {#if !question.deletedAt}
        <!-- Reuse existing QuestionContainer + QuestionRenderer markup inside a bordered sub-card -->
        <QuestionContainer ...>
...
7. When the last section is deleted, move all remaining questions to sectionless (set `exerciseSectionId` to `null`).
...
- Section title and color theme edits are reflected in the store.
- Section card header, content, and footer match the three-zone UI.
...
questionnaire.set({
  ...
  questions: allQuestions.map((q) => ({ ...q, exerciseSectionId: q.exerciseSectionId ?? null }))
});
```

3. Ensure `exerciseSectionId` is set on each question from the API response.
...
- Questions have correct `exerciseSectionId` values.
...
2. Create or reuse a read-only `SectionHeader` component for take/preview/view modes. This is separate from the edit-mode `SectionEditor`.
3. When sections exist, render questions grouped under section headers:
...
4. Questions are numbered within their section: "1.", "2." etc., resetting per section.

**Files to change**:
- `apps/dashboard/src/lib/features/course/components/exercise/preview.svelte`
- `packages/ui/src/custom/exercise-question/section-header.svelte` (if not already created by take-mode work)
- `packages/ui/src/custom/exercise-question/index.ts` (if creating the shared header)
...
if hasSectionedExercise:
  ...
  evaluateAfterBehavior(currentSection.afterBehavior):
    ...
    if action === 'go_to_section':
      → find target section by exerciseSectionId
...
Ticket 3.8 (Preview) — depends on Ticket 3.1
```
...
| --- | --- | --- |
| `exercise-section-types.ts` | **New** | Section model, color theme type, after-behavior types, display mode type |
...
| File | Status | Description |
| --- | --- | --- |
| `section-header.svelte` | **New** | Read-only section header for take/preview/view modes |
...
### `apps/dashboard/src/lib/features/course/components/exercise/`

| File | Status | Description |
| --- | --- | --- |
| `section-editor.svelte` | **New** | Three-zone section editing card: header, content, footer |
| `section-overview.svelte` | **New** | Section overview screen for student take mode |
| `section-delete-modal.svelte` | **New** | Section deletion confirmation |
