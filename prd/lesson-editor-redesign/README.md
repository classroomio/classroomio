...
### 1. UI implementation may drift from the intended design

**Risk:**
The implemented editor may differ from the intended layout, interactions, spacing, or component placement.
...
* Unified canvas with multiple content sections
* Add content state
* Settings slider open and closed
* Edit/Preview toggle
* Responsive behavior where applicable

The prototype is the visual and interaction reference for implementation. The PRD remains the source of truth for product behavior, scope, and technical constraints.

### 2. Settings slider may not work well on mobile

**Risk:**
A partial-width right-side slider may leave too little usable space on smaller screens.

**Mitigation:**
Use a full-width slider/sheet below an appropriate mobile breakpoint if necessary.

### 3. Removing tabs may make section navigation harder

**Risk:**
Long lessons may make it harder to quickly reach a specific content section compared with the current tabs.

**Mitigation:**
Do not introduce another navigation system in v1 unless testing shows it is necessary. An in-canvas jump-to-section control can be considered later, with `PageOutline` as a possible reusable component.

### 4. Preview could be misunderstood

**Risk:**
Admins may assume Preview represents the public course page, even though the in-app and public renderers currently have different capabilities.

**Mitigation:**
Clearly label the Preview state as the **in-app learner experience** and communicate the distinction, for example:

> Previewing as a logged-in learner. The public course page may render this differently.

### 5. Multiple content components may expose layout issues

**Risk:**
Mounting Video, Slide, Note, and Document together may expose issues that were not visible when each component was isolated inside a tab.
...
**Mitigation:**
Test all content components together and verify that existing add/edit modals and pickers continue to work correctly when multiple sections are mounted on the same page.
