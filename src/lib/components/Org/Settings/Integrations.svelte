<script>
  ...
  async function deleteChatId() {
    ...
    let { error } = await supabase.from('profile').update(updates).match({ id: $profile.id });
    // Check if telegram_chat_id is not null before attempting to delete
    if ($profile.telegram_chat_id !== null) {
      // Update the telegram_chat_id in the profile store to null
      profile.update((profileData) => ({
        ...profileData,
        telegram_chat_id: null
      }));
...
<Grid class="border-c rounded border-gray-200 w-full mt-5">
  <Row class="flex lg:flex-row flex-col items-center py-7 border-bottom-c">
    <Column sm={4} md={4} lg={4} class="flex flex-col items-center">
      <img src="/telegram-svg.svg" alt="" class="w-10" />
    ...
    <Column sm={8} md={8} lg={8} class="mt-2 lg:mt-0 flex flex-col items-center lg:items-start">
      {#if $profile.telegram_chat_id !== null}
        <div class="flex flex-col items-center">
          <Checkmark size={32} />
          <SectionTitle>Telegram Integration Successful</SectionTitle>
          <PrimaryButton className="mt-3" variant={VARIANTS.CONTAINED_DANGER} onClick={deleteChatId}
            >Unlink Telegram</PrimaryButton
          >
        </div>
      {:else}
        <div>
          <h3 class="font-normal text-lg">
            You can connect your account to telegram to get notified of any change to your account.
          </h3>
          <h4 class="font-normal">
            <b>Follow the below steps:</b>
          </h4>
          <h4 class="font-normal mt-[-1px]">
            Step 1: Authenticate your account via the ClassroomIO bot.
          </h4>
          <PrimaryButton
            className="mb-5"
            variant={VARIANTS.OUTLINED}
            onClick={() => window.open('https://t.me/classroomio_bot', '_blank')}
            >Open Bot</PrimaryButton
          >
          <h4 class="font-normal">Step 2: Enter the Chat ID given to you by the bot.</h4>
          <TextField bind:value={chatId} placeholder="360111" className="w-full lg:w-60 mb-5" />
