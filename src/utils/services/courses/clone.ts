...
import type { Course, Exercise, Lesson, Profile } from '../../types';
// @ts-ignore
...
async function cloneLessons(lessons: Lesson[], courseId: Course['id']) {
  ...
  const { data } = await supabase.from('lesson').insert(clonedLessons);
...
async function cloneExercises(lessons: Lesson[]) {
  ...
  for (const lesson of lessons) {
    ...
    for (const exerciseItem of exercise) {
      ...
      // Insert
      const { data } = await supabase.from('exercise').insert({
        description,
        due_by: new Date(),
        lesson_id: lessonId,
        title,
      ...
      for (const question of questions) {
        ...
        // Insert
        const { data } = await supabase.from('question').insert({
          name: question.name,
        ...
        if (QUESTION_TYPE.TEXTAREA !== newQuestion.question_type_id) {
          ...
          // Insert[Batch]
          await supabase.from('option').insert(newOptions);
        }
      }
    }
  }
